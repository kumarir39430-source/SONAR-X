...
function App() {
  ...
  const [sonarXLatency, setSonarXLatency] = useState(null);

  /* Segmentation — separate results for YOLO and Faster R-CNN */
  const [yoloSegmentationResult, setYoloSegmentationResult] = useState(null);
  const [yoloSegmentationLoading, setYoloSegmentationLoading] = useState(false);
  const [yoloSegmentationError, setYoloSegmentationError] = useState("");
  const [rcnnSegmentationResult, setRcnnSegmentationResult] = useState(null);
  const [rcnnSegmentationLoading, setRcnnSegmentationLoading] = useState(false);
  const [rcnnSegmentationError, setRcnnSegmentationError] = useState("");
  ...
  const handleFile = (file) => {
    ...
    setSonarXError("");
    setSonarXLatency(null);
    setAdvancedResult(null);
    setAdvancedLoading(false);
    setAdvancedError("");
    setYoloSegmentationResult(null);
    setYoloSegmentationError("");
    setRcnnSegmentationResult(null);
    setRcnnSegmentationError("");
  };
  ...
  /* =======================================================
     LEGACY DETECTION SEGMENTATION — YOLO / R-CNN -> SAM
     Each detector has its own segmentation state so the two
     results never overwrite each other.
     ======================================================= */
  const runSegmentation = async (analysisData, sourceFile, sourceModel) => {
    if (!analysisData || !sourceFile) return;

    const isRcnn = sourceModel === "Faster R-CNN";
    const setLoadingState = isRcnn ? setRcnnSegmentationLoading : setYoloSegmentationLoading;
    const setErrorState = isRcnn ? setRcnnSegmentationError : setYoloSegmentationError;
    const setResultState = isRcnn ? setRcnnSegmentationResult : setYoloSegmentationResult;

    setLoadingState(true);
    setErrorState("");

    try {
      const detections = normalizeDetections(analysisData);
      if (!detections.length) {
        setResultState(null);
        setErrorState(`No ${sourceModel} detections were returned, so there is nothing to segment.`);
        return;
      }
      ...
      formData.append("source_model", sourceModel);

      const response = await fetch(`${API_URL}/segmentation/segment`, {
        method: "POST",
        body: formData,
      });
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(data?.detail || `Segmentation backend returned ${response.status}`);
      }

      setResultState(data);
    } catch (error) {
      console.error(`${sourceModel} segmentation failed:`, error);
      setErrorState(
        error?.message ||
          `${sourceModel} segmentation could not be completed. Check the SAM backend.`
      );
      setResultState(null);
    } finally {
  ...
  const analyzeSonarX = async () => {
    ...
    const sonarXStartedAt = performance.now();
    ...
    try {
      ...
      setSonarXLatency(performance.now() - sonarXStartedAt);

      // Feature 3 consumes the completed SONAR-X result using the same
      // existing Advanced Marine Intelligence workflow.
  ...
  const analyzeFile = async () => {
    ...
    try {
      ...
      if (yoloResult.status === "fulfilled") {
        ...
        runAdvancedIntelligence(data, selectedFile);
        runSegmentation(data, selectedFile, "YOLO11n");
      } else {
      ...
      if (rcnnResultValue.status === "fulfilled") {
        ...
        runSegmentation(rcnnResultValue.value.data, selectedFile, "Faster R-CNN");
      } else {
          ...
          <div className="page-content">
            ...
            {page === "Analyze Sonar" && (
              <AnalyzePage
                ...
                analyzeSonarX={analyzeSonarX}
                sonarXResult={sonarXResult}
                sonarXLoading={sonarXLoading}
                sonarXError={sonarXError}
                sonarXLatency={sonarXLatency}
                yoloSegmentationResult={yoloSegmentationResult}
                yoloSegmentationLoading={yoloSegmentationLoading}
                yoloSegmentationError={yoloSegmentationError}
                rcnnSegmentationResult={rcnnSegmentationResult}
                rcnnSegmentationLoading={rcnnSegmentationLoading}
                rcnnSegmentationError={rcnnSegmentationError}
              />
            )}
...
function AnalyzePage({
  ...
  yoloSegmentationLoading,
  yoloSegmentationError,
  rcnnSegmentationResult,
  rcnnSegmentationLoading,
  rcnnSegmentationError,
}) {
    ...
    <>
          ...
          <div className="card-body">
            {!(analyzed || sonarXResult) ? (
              <div className="empty-state">
                ...
                <p className="small">
                  Upload a sonar image and click
                  Analyze with YOLO + R-CNN or SONAR-X.
                </p>
              </div>
            ) : (
              <>
                {(sonarXResult?.annotated_image || annotatedImage) ? (
                  <img
                    className="result-image"
                    src={
                      sonarXResult?.annotated_image
                        ? getAnnotatedUrl(sonarXResult.annotated_image)
                        : annotatedImage
                    }
                    alt={
                      sonarXResult?.annotated_image
                        ? "SONAR-X annotated sonar"
                        : "YOLO annotated sonar"
                    }
                  />
                ) : (
                  <div className="empty-state">
                    Annotated image was not returned by the backend.
                  </div>
                )}
                ...
                <div className="result-grid">
                  <div className="result-box">
                    ...
                    <strong>
                      {sonarXResult
                        ? (sonarXResult.objects_detected ?? sonarXResult.count ?? 0)
                        : detections.length}
                    </strong>
                  </div>
                  ...
                  <div className="result-box">
                    ...
                    <strong>
                      {sonarXResult
                        ? `${sonarXResult.highest_confidence ?? 0}%`
                        : detections.length
                          ? `${Math.round(
                              detections.reduce(
                                (sum, d) =>
                ...
                {sonarXResult && (
                  <div className="gps-box" style={{ marginTop: 12 }}>
                    <Layers size={18} />
                    <div>
                      <strong style={{ fontSize: 13 }}>
                        SONAR-X result included
                      </strong>
                      <div className="small" style={{ marginTop: 3 }}>
                        YOLO11n-Seg • Detection + Instance Segmentation •{" "}
                        {sonarXResult.objects_detected ?? sonarXResult.count ?? 0} target(s)
                      </div>
                    </div>
                  </div>
                )}
      ...
      {analyzed && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="card-header">
            <div>
              <div className="card-title">YOLO11n — Pixel Segmentation</div>
              <div className="muted small" style={{ marginTop: 4 }}>
                YOLO bounding boxes are sent to the existing SAM segmentation endpoint to generate pixel masks.
              </div>
            </div>
            <span className="badge">{yoloSegmentationLoading ? "Segmenting..." : yoloSegmentationResult?.success ? "SAM Complete" : "SAM"}</span>
          </div>
          <div className="card-body">
            {yoloSegmentationLoading ? (
              <div className="empty-state"><Loader2 size={24} className="spin" /><strong style={{ marginTop: 8 }}>Generating YOLO pixel masks...</strong><p className="small">YOLO detection is complete. SAM is refining the detected regions.</p></div>
            ) : yoloSegmentationError ? (
              <div className="empty-state"><strong>YOLO segmentation unavailable</strong><p className="small">{yoloSegmentationError}</p></div>
            ) : yoloSegmentationResult?.segmented_image ? (
              <>
                <img className="result-image" loading="eager" decoding="async" src={getAnnotatedUrl(yoloSegmentationResult.segmented_image)} alt="YOLO11n SAM pixel segmentation result" />
                <div className="result-grid" style={{ marginTop: 12 }}>
                  <div className="result-box"><span className="muted small">Objects segmented</span><strong>{yoloSegmentationResult.objects_segmented ?? 0}</strong></div>
                  <div className="result-box"><span className="muted small">Method</span><strong style={{ fontSize: 14 }}>YOLO11n → SAM</strong></div>
                  <div className="result-box"><span className="muted small">Output</span><strong style={{ fontSize: 14 }}>Pixel masks + boxes</strong></div>
                </div>
              </>
            ) : (
              <div className="empty-state"><strong>Waiting for YOLO segmentation</strong><p className="small">Run Analyze Sonar to generate the mask.</p></div>
            )}
          </div>
        </div>
      )}
      ...
      {analyzed && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="card-header">
            <div>
              <div className="card-title">Faster R-CNN — Pixel Segmentation</div>
              <div className="muted small" style={{ marginTop: 4 }}>
                Faster R-CNN detections are independently sent to the same SAM segmentation pipeline.
              </div>
            </div>
            <span className="badge">{rcnnSegmentationLoading ? "Segmenting..." : rcnnSegmentationResult?.success ? "SAM Complete" : "SAM"}</span>
          </div>
          <div className="card-body">
            {rcnnSegmentationLoading ? (
              <div className="empty-state"><Loader2 size={24} className="spin" /><strong style={{ marginTop: 8 }}>Generating R-CNN pixel masks...</strong><p className="small">Faster R-CNN detection is complete. SAM is refining the detected regions.</p></div>
            ) : rcnnSegmentationError ? (
              <div className="empty-state"><strong>R-CNN segmentation unavailable</strong><p className="small">{rcnnSegmentationError}</p></div>
            ) : rcnnSegmentationResult?.segmented_image ? (
              <>
                <img className="result-image" loading="eager" decoding="async" src={getAnnotatedUrl(rcnnSegmentationResult.segmented_image)} alt="Faster R-CNN SAM pixel segmentation result" />
                <div className="result-grid" style={{ marginTop: 12 }}>
                  <div className="result-box"><span className="muted small">Objects segmented</span><strong>{rcnnSegmentationResult.objects_segmented ?? 0}</strong></div>
                  <div className="result-box"><span className="muted small">Method</span><strong style={{ fontSize: 14 }}>Faster R-CNN → SAM</strong></div>
                  <div className="result-box"><span className="muted small">Output</span><strong style={{ fontSize: 14 }}>Pixel masks + boxes</strong></div>
                </div>
              </>
            ) : (
              <div className="empty-state"><strong>Waiting for R-CNN segmentation</strong><p className="small">The R-CNN result will be segmented independently when detection completes.</p></div>
            )}
          </div>
        </div>
      )}
      ...
      {(analyzed || sonarXResult) && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="card-header">
            ...
            <span className="badge">YOLO11n vs Faster R-CNN vs SONAR-X</span>
          </div>
          <div className="card-body">
            {(() => {
              ...
              return (
                <>
                  <div className="result-grid">
                    ...
                    <div className="result-box">
                      <span className="muted small">SONAR-X</span>
                      {sonarXLoading ? <strong>Analyzing...</strong> : sonarXResult ? (
                        <>
                          <strong>{sonarXResult.objects_detected ?? sonarXResult.count ?? 0} targets</strong>
                          <div className="muted small" style={{ marginTop: 5 }}>
                            Highest confidence: {sonarXResult.highest_confidence ?? 0}%
                          </div>
                          <div className="muted small" style={{ marginTop: 3 }}>
                            YOLO11n-Seg • Segmentation enabled
                          </div>
                        </>
                  ...
                  <div className="card" style={{ marginTop: 16, border: "1px solid #071C29", boxShadow: "none" }}>
                    ...
                    <div className="card-body">
                      <div className="result-grid">
                        ...
                        <div className="result-box">
                          <span className="muted small">SONAR-X time</span>
                          <strong>{formatLatency(sonarXLatency)}</strong>
                          <div className="muted small" style={{ marginTop: 5 }}>Time complexity: {getTimeComplexity("YOLO11n")}</div>
                          <div className="muted small" style={{ marginTop: 3 }}>YOLO11n single-stage detection + segmentation; processing grows with image and feature-map size.</div>
                        </div>
                        <div className="result-box">
