...
function App() {
  ...
  /* New SONAR-X YOLO11n-Seg analysis (keeps legacy YOLO + R-CNN intact) */
  const [sonarXResult, setSonarXResult] =
    useState(null);
  const [sonarXLoading, setSonarXLoading] =
    useState(false);
  const [sonarXError, setSonarXError] =
    useState("");
  ...
  const analyzeFile = async () => {
    ...
    /* New SONAR-X segmentation pipeline */
    const runSonarX = async () => {
      const startedAt = performance.now();
      ...
      if (!response.ok) {
        throw new Error(`SONAR-X backend returned ${response.status}`);
      }
      return {
    ...
    try {
      const [yoloResult, rcnnResultValue, sonarXResultValue] =
      ...
      if (sonarXResultValue.status === "fulfilled") {
        setSonarXResult(sonarXResultValue.value.data);
        setSonarXError("");
      } else {
        console.error("SONAR-X analysis failed:", sonarXResultValue.reason);
        setSonarXError(
          sonarXResultValue.reason?.message ||
          "SONAR-X segmentation analysis could not be completed."
        );
      }
    } finally {
  ...
  return (
    <style>{`
      * {
      ...
      .twin-object-label { position: absolute; top: 35px; left: 50%; transform: translateX(-50%); white-space: nowrap; font-size: 10px; font-weight: 800; color: #e0f2fe; text-shadow: 0 1px 4px #020617; pointer-events: none; }
      ...
      .twin-empty span { color: #94a3b8; font-size: 12px; line-height: 1.5; }
    ...
    <>
      <div className="page-heading">
        <div>
          ...
          <p>
            Expert-in-the-loop analysis grounded in SONAR-X
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                Live context from the existing SONAR-X
                session.
              </p>
            </div>
          </div>
            ...
            >
              {getLatLon(analysisResult?.geotag || analysisResult?.location || currentLocation) ? (
                ...
                >
                  ...
                  >
                    <Popup>
                      SONAR-X survey location. This marker represents the scan coordinate, not an exact target position.
                    </Popup>
                  </Marker>
                </MapContainer>
              ) : (
            ...
            >
              <div>
                ...
                >
                  ...
                  SONAR-X context and curated marine-sonar
                  knowledge.
                </p>
              </div>
            </div>
...
function AnalyzePage({
  ...
  return (
    <>
          ...
          <div className="card-body">
            ...
            >
              {loading ? (
                ...
                <>
                  <Target size={17} />
                  Analyze with SONAR-X + YOLO + R-CNN
                </>
              )}
            </button>
          </div>
      ...
      {(sonarXLoading || sonarXResult || sonarXError) && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="card-header">
            <div>
              <div className="card-title">SONAR-X AI — Detection + Segmentation</div>
              <div className="muted small" style={{ marginTop: 4 }}>
                YOLO11n-Seg using the dedicated SONAR-X 3-class model at 1024px inference.
              </div>
            </div>
            <span className="badge">
              {sonarXLoading ? "Analyzing..." : sonarXResult ? "Complete" : "Unavailable"}
            </span>
          </div>
          ...
          <div className="card-body">
            {sonarXLoading ? (
              <div className="empty-state">
                <Loader2 size={24} />
                <strong style={{ marginTop: 8 }}>Running SONAR-X segmentation...</strong>
                <p className="small">
                  The uploaded image is being processed for Fishing Net, Pipe and Shipwreck.
                </p>
              </div>
            ) : sonarXError ? (
              <div className="gps-box gps-error">
                <div>
              ...
              <>
                ...
                <div className="result-grid" style={{ marginTop: 12 }}>
                  <div className="result-box">
                    ...
                    <strong>{sonarXResult.objects_detected ?? sonarXResult.count ?? normalizeDetections(sonarXResult).length}</strong>
                  </div>
                  <div className="result-box">
                    <span className="muted small">Highest confidence</span>
                    <strong>{Number.isFinite(Number(sonarXResult.highest_confidence)) ? `${Number(sonarXResult.highest_confidence).toFixed(1)}%` : "—"}</strong>
                  </div>
                  <div className="result-box">
                    <span className="muted small">Inference</span>
                    <strong>{sonarXResult.inference_size ? `${sonarXResult.inference_size}px` : "—"}</strong>
                  </div>
                </div>
                ...
                <div className="card" style={{ marginTop: 16, boxShadow: "none" }}>
                  ...
                  <div className="card-body table-wrap">
                    {normalizeDetections(sonarXResult).length === 0 ? (
                      <div className="empty-state">No SONAR-X targets were detected.</div>
                    ) : (
                      <table>
                        <thead>
                          <tr>
                        ...
                        <tbody>
                          {normalizeDetections(sonarXResult).map((detection, index) => (
                            <tr key={`sonarx-${detection._index || index}`}>
                              <td>{index + 1}</td>
                              <td><span className="badge">{detection.class_name}</span></td>
                              <td>{Math.round(detection.confidence_percent)}%</td>
                ...
                <div className="gps-box" style={{ marginTop: 15 }}>
                  ...
                  <div>
                    ...
                    <div className="small" style={{ marginTop: 3 }}>
                      {getLocationText(sonarXResult.geotag || sonarXResult.location)}
                    </div>
                  </div>
                </div>
              </>
    ...
    <>
      <div className="page-heading">
        <div>
          ...
          <p>
            Compare recorded SONAR-X survey detections across two scans without inventing missing historical evidence.
          </p>
        </div>
      </div>
...
function DigitalTwinPage({ analysisResult, surveyScans = [] }) {
  ...
  return (
    <>
      ...
      <div className="gps-box" style={{ marginTop: 18 }}><div><strong style={{ fontSize: 13 }}>Prototype boundary</strong><div className="small" style={{ marginTop: 4 }}>This first Digital Twin version uses real detected objects and survey GPS from SONAR-X. Accurate seafloor elevation, object depth and exact target coordinates require bathymetry or sonar georeferencing data.</div></div></div>
    </>
  );
}
          ...
          {validScans.map((scan, index) => {
            ...
            return (
              <React.Fragment key={`${scan.id || "scan"}-${index}`}>
                ...
                >
                  <Popup>
                    <strong>SONAR-X Survey Scan #{index + 1}</strong>
                    <br />
