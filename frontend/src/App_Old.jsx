      ...
      <div className="card-body">
        {loading && (
          <div className="gps-box" style={{ marginBottom: 14 }}>
            <Loader2 size={17} />
            <div>
              <strong style={{ fontSize: 13 }}>Running advanced intelligence...</strong>
              <div className="small" style={{ marginTop: 3 }}>Building evidence, seabed context, risk, fingerprints and survey recommendations.</div>
            </div>
          </div>
        )}
        {error && (
          <div className="gps-box gps-error" style={{ marginBottom: 14 }}>
            <ShieldAlert size={17} />
            <div>
              <strong style={{ fontSize: 13 }}>Feature 3 could not be completed</strong>
          ...
          <>
            ...
            {detections.length > 0 && (
              <div className="table-wrap" style={{ marginTop: 16 }}>
                <table><thead><tr><th>#</th><th>Fingerprint</th><th>Confidence</th><th>Evidence</th><th>Risk</th><th>Priority</th></tr></thead>
                  <tbody>{detections.map((detection, index) => { const evidence=detection.evidence||{}; const risk=detection.risk||{}; return <tr key={`advanced-${index}`}><td>{index+1}</td><td><strong>{detection.fingerprint_id || "—"}</strong></td><td>{Number.isFinite(Number(detection.confidence_percent)) ? `${Math.round(Number(detection.confidence_percent))}%` : "—"}</td><td>{evidence.strength || evidence.evidence_strength || "—"}</td><td>{risk.risk_level || "—"}{Number.isFinite(Number(risk.risk_score)) ? ` (${Math.round(Number(risk.risk_score))})` : ""}</td><td>{risk.priority || "—"}</td></tr>; })}</tbody>
                </table>
              </div>
            )}
            ...
            <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
              <div style={{ padding: 14, borderRadius: 12, background: "#f8fafc", border: "1px solid #e2e8f0" }}><strong style={{ fontSize: 13 }}>AI Next-Survey Planner</strong><div className="small" style={{ marginTop: 6, lineHeight: 1.55 }}>{surveyPlan.recommendation || surveyPlan.summary || "Prototype survey recommendation generated from current evidence."}</div></div>
              <div style={{ padding: 14, borderRadius: 12, background: "#f8fafc", border: "1px solid #e2e8f0" }}><strong style={{ fontSize: 13 }}>Cleanup Mission Planner</strong><div className="small" style={{ marginTop: 6, lineHeight: 1.55 }}>{cleanup.recommendation || cleanup.summary || "Prototype mission-planning guidance; not an operational navigation route."}</div></div>
            </div>
            <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: "#fff7ed", border: "1px solid #fed7aa" }}><div className="small" style={{ lineHeight: 1.55 }}><strong>Prototype scope:</strong> Feature 3 outputs are evidence-aware decision-support prototypes. Acoustic-shadow, seabed, risk, hotspot, cleanup and predictive outputs are not validated physical, pollution, hazard or operational measurements. Synthetic Sonar Laboratory remains planned.</div></div>
          </>
        )}
      </div>
    </div>
