  ...
  const analyzeFile = async () => {
    ...
    try {
      ...
      if (rcnnResultValue.status === "fulfilled") {
        setRcnnResult(rcnnResultValue.value);
      } else {
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <ModelRow
              icon={Target}
              name="R-CNN Detector"
              description="Object detection comparison model"
              status="Ready"
            />
          ...
          <div className="card-body">
            <div className="result-grid">
              <div className="result-box">
                ...
                <strong>{detections.length} targets</strong>
                <div className="muted small" style={{ marginTop: 5 }}>
                  Avg. confidence: {detections.length ? `${Math.round(detections.reduce((sum, d) => sum + d.confidence_percent, 0) / detections.length)}%` : "0%"}
                </div>
              </div>
              <div className="result-box">
                <span className="muted small">Faster R-CNN</span>
                {rcnnLoading ? <strong>Analyzing...</strong> : rcnnResult ? (() => {
                  const rd = normalizeDetections(rcnnResult);
                  return <>
                    <strong>{rd.length} targets</strong>
                    <div className="muted small" style={{ marginTop: 5 }}>
                      Avg. confidence: {rd.length ? `${Math.round(rd.reduce((sum, d) => sum + d.confidence_percent, 0) / rd.length)}%` : "0%"}
                    </div>
                  </>;
