...
function App() {
  ...
  /* Segmentation — additive to the existing detection pipeline */
  ...
  const runAdvancedIntelligence = async (analysisData, sourceFile) => {
    ...
    try {
      ...
      formData.append("source_model", "YOLO11n");
      formData.append("source_detection_count", String(normalizeDetections(analysisData).length));
  ...
  /* =======================================================
     SEGMENTATION — YOLO BOX PROMPT -> SAM MASK
     Keeps all existing YOLO/R-CNN functionality unchanged.
     ======================================================= */
  const runSegmentation = async (analysisData, sourceFile) => {
    if (!analysisData || !sourceFile) return;
    ...
    try {
      ...
      formData.append(
        "detections",
        JSON.stringify(normalizeDetections(analysisData))
      );
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || `Segmentation backend returned ${response.status}`
        );
      }
      ...
      setSegmentationError(
        error?.message ||
          "Segmentation could not be completed. Check the SAM backend."
      );
      setSegmentationResult(null);
    } finally {
      setSegmentationLoading(false);
  ...
  const analyzeFile = async () => {
    ...
    try {
      // FAST JUDGE MODE: show YOLO results immediately.
      // R-CNN, Feature 3 and segmentation continue in the background.
      const yoloResult = await runYolo();

      const { data, elapsedMs } = yoloResult;
      ...
      // Background jobs — none of these block the YOLO result.
      runAdvancedIntelligence(data, selectedFile);
      runSegmentation(data, selectedFile);

      // R-CNN comparison runs independently after the main result is visible.
      runRcnn()
        .then(({ data: rcnnData, elapsedMs: rcnnElapsed }) => {
          setRcnnLatency(rcnnElapsed);
          setRcnnResult(rcnnData);
        })
        .catch((error) => {
          console.error("R-CNN analysis failed:", error);
          setRcnnError(error?.message || "R-CNN analysis could not be completed.");
        })
        .finally(() => setRcnnLoading(false));
    } catch (error) {
      console.error("YOLO analysis failed:", error);
      setAnalysisDuration(performance.now() - analysisStartedAt);
      setLoading(false);
      setRcnnLoading(false);
      alert(error?.message || "YOLO analysis failed. Make sure your FastAPI backend is running at http://127.0.0.1:8000.");
    }
  };
...
function AnalyzePage({
  ...
  // Show the uploaded image immediately while the backend annotated image loads.
  ...
  const displayAnalysisImage =
    annotatedImage && annotatedLoaded
      ? annotatedImage
      : localPreviewUrl || annotatedImage;
    ...
    <>
        ...
        <div className="card">
          ...
          <div className="card-body">
            {!analyzed ? (
              ...
              <>
                {displayAnalysisImage ? (
                  <>
                    <img
                      className="result-image"
                      src={displayAnalysisImage}
                      alt={
                        annotatedLoaded
                          ? "YOLO annotated sonar"
                          : "Uploaded sonar image"
                      }
                    />
                    {annotatedImage && !annotatedLoaded && (
                      <img
      ...
      {analyzed && (
        <div className="card" style={{ marginTop: 18 }}>
          <div className="card-header">
            <div>
              <div className="card-title">Pixel-Level Segmentation</div>
              <div className="muted small" style={{ marginTop: 4 }}>
                YOLO bounding boxes are retained and used as prompts for SAM to refine each target into a pixel-level mask.
              </div>
            </div>
            <span className="badge">
              {segmentationLoading ? "Segmenting..." : segmentationResult?.success ? "SAM Complete" : "SAM"}
            </span>
          </div>
          <div className="card-body">
            {segmentationLoading ? (
              <div className="empty-state">
                ...
                <strong style={{ marginTop: 8 }}>Generating pixel masks...</strong>
                <p className="small">YOLO detection is complete. SAM is refining the detected regions.</p>
              </div>
            ) : segmentationError ? (
              <div className="empty-state">
                <strong>Segmentation unavailable</strong>
                <p className="small">{segmentationError}</p>
              </div>
            ) : segmentationResult?.segmented_image ? (
              <>
                <img
                  className="result-image"
                  loading="eager"
                  decoding="async"
                  src={getAnnotatedUrl(segmentationResult.segmented_image)}
                  alt="SAM pixel-level segmentation over YOLO detections"
                />
                <div className="result-grid" style={{ marginTop: 12 }}>
                  <div className="result-box">
                    <span className="muted small">Objects segmented</span>
                    <strong>{segmentationResult.objects_segmented ?? 0}</strong>
                  </div>
                  <div className="result-box">
                    <span className="muted small">Method</span>
                    <strong style={{ fontSize: 14 }}>YOLO → SAM</strong>
                  </div>
                  <div className="result-box">
                    <span className="muted small">Output</span>
                    <strong style={{ fontSize: 14 }}>Pixel masks + boxes</strong>
                  </div>
                </div>
                <div className="small" style={{ marginTop: 10, lineHeight: 1.5 }}>
                  <strong>Prototype note:</strong> SAM is being used as a box-prompted segmentation model. Mask quality should be validated with sonar-specific segmentation ground truth before scientific or operational use.
                </div>
              </>
            ) : (
              <div className="empty-state">
                <strong>Waiting for segmentation</strong>
                <p className="small">Segmentation will appear after YOLO returns detections.</p>
              </div>
            )}
          </div>
        </div>
...
/* =========================================================
   FEATURE 3 — ADVANCED MARINE INTELLIGENCE & DECISION SYSTEM
   ========================================================= */
function AdvancedIntelligencePanel({ result, loading, error, yoloAnalysis }) {
  const [tab, setTab] = useState("overview");
  const yoloDetections = normalizeDetections(yoloAnalysis);
  const yoloSourceModel = yoloAnalysis?.model || yoloAnalysis?.source_model || "YOLO11n";
  ...
  return (
    <div className="card" style={{ marginTop: 18 }}>
      ...
      <div className="card-body">
        <div style={{ marginBottom: 14, padding: 14, borderRadius: 12, background: "linear-gradient(135deg, rgba(0,229,255,.10), rgba(0,120,180,.08))", border: "1px solid rgba(0,229,255,.28)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <Target size={17} />
                <strong style={{ fontSize: 14 }}>YOLO11n → Feature 3 Intelligence Pipeline</strong>
              </div>
              <div className="small" style={{ marginTop: 5 }}>Feature 3 is using the detections produced by the YOLO detector for evidence-aware marine intelligence.</div>
            </div>
            <span className="badge" style={{ borderColor: "rgba(53,229,138,.35)", color: "#35E58A" }}>CONNECTED</span>
          </div>
          <div className="result-grid" style={{ marginTop: 12 }}>
            <div className="result-box"><span className="muted small">Source detector</span><strong>{yoloSourceModel}</strong><div className="muted small" style={{ marginTop: 4 }}>Actual detection output</div></div>
            <div className="result-box"><span className="muted small">YOLO objects sent</span><strong>{yoloDetections.length}</strong><div className="muted small" style={{ marginTop: 4 }}>{loading ? "Waiting for Feature 3 analysis" : "Received by Feature 3"}</div></div>
            <div className="result-box"><span className="muted small">Bounding boxes</span><strong>{yoloDetections.filter(d => d.bbox || d.bounding_box).length}</strong><div className="muted small" style={{ marginTop: 4 }}>Retained as object evidence</div></div>
            <div className="result-box"><span className="muted small">Intelligence layer</span><strong>Feature 3</strong><div className="muted small" style={{ marginTop: 4 }}>Risk + evidence + decisions</div></div>
          </div>
          {yoloDetections.length > 0 && (
            <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 7 }}>
              {yoloDetections.slice(0, 8).map((d, i) => (
                <span key={`yolo-source-${i}`} className="badge">YOLO: {d.class_name || "Unknown"} · {Math.round(Number(d.confidence_percent || 0))}%</span>
              ))}
            </div>
          )}
        </div>
