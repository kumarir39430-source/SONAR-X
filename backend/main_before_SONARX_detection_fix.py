...
# Support both ways of starting the backend:
#   1. From backend folder:  uvicorn main:app --reload
#   2. From project root:     uvicorn backend.main:app --reload
# The backend is started from the backend folder, so use local package imports.
# Keeping these imports local also prevents the "No module named 'backend'"
# error when running: uvicorn main:app --reload
...
# Keep this model unchanged for the existing dataset/features.
# --------------------------------------------------------------------
...
# This is kept separate from the legacy model so both datasets work.
# The model was trained for: Fishing Net, Pipe, Shipwreck.
# --------------------------------------------------------------------
...
# Legacy model: existing 4-class detector
model = YOLO(MODEL_PATH)

print("YOLO legacy model loaded successfully.")
print("Legacy model classes:", model.names)

# New SONAR-X model: YOLO11n-Seg 3-class model
if not os.path.exists(SONAR_X_MODEL_PATH):
    raise FileNotFoundError(
        f"SONAR-X YOLO11n-Seg model not found at: {SONAR_X_MODEL_PATH}"
    )

print("==========================================")
print("Loading SONAR-X YOLO11n-Seg model...")
print("Model path:", SONAR_X_MODEL_PATH)
print("==========================================")

sonar_x_model = YOLO(SONAR_X_MODEL_PATH)

print("SONAR-X YOLO11n-Seg model loaded successfully.")
print("SONAR-X model classes:", sonar_x_model.names)
...
# Existing dataset/model classes. DO NOT replace these with the new classes.
CLASS_NAMES = {
    0: "Aircraft",
    1: "Fish",
    2: "Rocks/Stones",
...
# New SONAR-X combined dataset/model classes.
SONAR_X_CLASS_NAMES = {
    0: "Fishing Net",
    1: "Pipe",
    2: "Shipwreck",
...
# ============================================================
# This endpoint uses the NEW 3-class segmentation model only.
# The existing /analyze and /detect endpoints continue using the
# legacy 4-class model, so the old dataset remains fully supported.
#
# Demo configuration:
#   imgsz = 1024
#   conf  = 0.25
...
async def analyze_sonar_x(
    file: UploadFile = File(...),
    latitude: float = Form(DEFAULT_LATITUDE),
    longitude: float = Form(DEFAULT_LONGITUDE),
):
    ...
    try:
        ...
        # 1024 gives better localization on high-resolution sonar
        # images without retraining the model.
        # --------------------------------------------------------
        ...
        if result.boxes is not None:
            for index, box in enumerate(result.boxes):
                ...
                class_name = SONAR_X_CLASS_NAMES.get(
                    class_id,
                ...
                # YOLO11-Seg produces a mask for segmentation detections.
                mask_available = (
        ...
        # Keep browser display lightweight.
        max_display_width = 1400
        if annotated_image.width > max_display_width:
            scale = max_display_width / annotated_image.width
            annotated_image = annotated_image.resize(
                (
                    max_display_width,
                    max(1, int(annotated_image.height * scale))
                ),
                Image.Resampling.LANCZOS,
            )
        ...
        scan_id = f"SONAR-X-{uuid.uuid4().hex[:8].upper()}"
        ...
        geotag = {
            ...
            "source": (
                ...
                if (
                    latitude != DEFAULT_LATITUDE
                    or longitude != DEFAULT_LONGITUDE
                )
                else "Survey Location"
            ),
            "accuracy": "Prototype GPS location",
        }

        return {
            "success": True,
            "status": "SONAR-X Segmentation Analysis Completed",
            "scan_id": scan_id,
            "filename": file.filename,
            "model": "YOLO11n-Seg SONAR-X",
            "model_path": SONAR_X_MODEL_PATH,
            ...
            "message": (
                "SONAR-X YOLO11n-Seg analysis completed successfully. "
                "Detection and segmentation results are available."
            ),
        }

    except Exception as exc:
        print("SONAR-X SEGMENTATION ANALYSIS ERROR:", str(exc))

        return {
            "success": False,
            "status": "SONAR-X Segmentation Analysis Failed",
            "error": str(exc),
            "objects_detected": 0,
            "count": 0,
            "detections": [],
